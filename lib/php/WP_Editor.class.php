...
class WP_Editor {
	...
	function editor( $content, $editor_id, $settings = array(), $media_buttons = true ) {
		...
		$set = wp_parse_args( $settings,  array(
			...
			'wp_buttons_css' => get_template_directory_uri() . '/lib/css/wp-editor/editor-buttons.css', // styles for both visual and HTML editors buttons
		...
		if ( $can_richedit || $media_buttons ) {
			...
			if ( $can_richedit ) {
				...
				$toolbar .= '<a id="' . $id . '-html" class="' . $html_class . 'hide-if-no-js wp-switch-editor" onclick="wpEditor.s(this);return false;">' . __('HTML','cosmotheme') . "</a>\n";
				$toolbar .= '<a id="' . $id . '-tmce" class="' . $tmce_class . 'hide-if-no-js wp-switch-editor" onclick="wpEditor.s(this);return false;">' . __('Visual','cosmotheme') . "</a>\n";
			}
